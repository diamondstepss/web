...
export function HomePage({
  ...
  /**
   * What the admin's section builder says this homepage should be.
   *
   * Empty means nothing is configured, or the fetch failed — in which case the
   * built-in layout runs instead. A homepage that rendered nothing because a
   * table was briefly unreachable would be a worse failure than one that
   * ignores a setting.
   */
  ...
  /**
   * One renderer for every product row.
   *
   * There were three copies of this markup — best discounts, new arrivals and
   * sale — differing only in heading, link and which array they sliced. Three
   * copies meant a spacing change had to be made three times, and the builder
   * could only ever drive rails the page already hardcoded.
   */
  const renderRail = (title: string, href: string, items: Product[]) =>
    items.length === 0 ? null : (
          <section className="section-pad">
            <div className="mx-auto max-w-[1440px] px-6">
  ...
  /** Where "view all" goes, given what the rail is showing. */
  const railHref = (s: HomeSection) => {
    if (s.source === 'SALE') return '/product-category/sale'
    if (s.sourceSlug) return `/product-category/${s.sourceSlug}`
    return '/shop'
  }

  /**
   * A configured section, rendered by type.
   *
   * Unknown types return null rather than throwing: the list is a database
   * constraint, and a value added there before this file knows about it should
   * leave a gap, not break the homepage.
   */
  const renderSection = (s: HomeSection) => {
    switch (s.type) {
      case 'HERO_SLIDER':    return HERO
      case 'USP_STRIP':      return USP
      case 'CATEGORY_TILES': return CATEGORIES
      case 'VIDEO_HERO':     return VIDEO
      case 'BRAND_STRIP':    return BRAND_STRIP
      case 'SPLIT_BANNER':   return SPLIT
      case 'NEWSLETTER':     return VIDEO_GRID
      case 'PRODUCT_RAIL':   return renderRail(s.title, railHref(s), s.products)
      default:               return null
    }
  }

  /**
   * The built-in layout, for a shop that has never opened the builder.
   *
   * Verbatim, so nothing changes for them.
   */
  const BUILT_IN = (
    <>
      {HERO}
      {USP}
      {renderRail('BEST DISCOUNTS', '/product-category/sale', uniqueById([...PRODUCTS, ...SALE_PRODUCTS]).slice(0, 8))}
      {CATEGORIES}
      {VIDEO}
      {BRAND_STRIP}
      {renderRail('NEW ARRIVALS', '/product-category/new-arrivals', PRODUCTS.slice(0, 8))}
      {SPLIT}
      {VIDEO_GRID}
      {renderRail('SALE', '/product-category/sale', uniqueById([...SALE_PRODUCTS, ...PRODUCTS.slice(0, 4)]))}
    </>
  )
  ...
  return (
    <div>
      {sections.length > 0
        ? sections.map((s) => <Fragment key={s.id}>{renderSection(s)}</Fragment>)
        : BUILT_IN}
